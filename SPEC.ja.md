...
キーボードショートカットで起動した場合（マウスホバーではなく）、フルキーボードナビゲーションをサポート。

### アクティベーション

- `activateViaKeyboard()` がキーボードショートカットで Keypunch が起動された際に呼ばれる
- `triggerPanel.allowBecomeKey = true` を設定し、キーウィンドウにする
...
- キーボードでパネルを開く場合: トリガー `allowBecomeKey = false`、拡張 `allowBecomeKey = true`、拡張 `makeKey()`
- パネルを閉じる場合: キーボード駆動だった場合、トリガー `allowBecomeKey = true`、トリガー `makeKey()` (フォーカスがトリガーに戻る)
...
`KeyablePanel` は `NSPanel` サブクラスで、トグル可能な `canBecomeKey` (`allowBecomeKey` プロパティ) によりショートカット録画時とキーボードナビゲーション時のキーボードフォーカスを有効化。
