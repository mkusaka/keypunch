...
**矢印キーナビゲーション (上下)**: 上下矢印でアプリ行間のみを移動 (編集ボタンをスキップ、ラップ)。フォーカスがない状態では、下矢印で最初の行 (空なら Add App)、上矢印で Add App にフォーカス。編集モード中は無効。
...
5. 非編集モードでフォーカスあり → フォーカスをクリア (初期状態に戻る)
...
| `testEscClearsFocusInNonEditMode` | 非編集モードで Esc するとフォーカスがクリアされ初期状態に戻る |
...
| `testDownArrowFromNoFocusFocusesFirstRow` | フォーカスなしで下矢印 → 最初の行にフォーカス |
| `testUpArrowFromNoFocusFocusesAddApp` | フォーカスなしで上矢印 → Add App にフォーカス |
| `testDownArrowFromNoFocusEmptyListFocusesAddApp` | 空リストで下矢印 → Add App にフォーカス |
