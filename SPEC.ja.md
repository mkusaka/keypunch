...
│   ├── FloatingTriggerView.swift    # トリガーピル (4つのアイコンボタン、ツールチップ)
...
| アイコン | SF Symbol | 動作 | ツールチップ |
|---------|-----------|------|------------|
| キーボード | `keyboard` | 拡張パネルをトグル | "Toggle Keypunch" |
| 終了 | `rectangle.portrait.and.arrow.right` | アプリを終了 | "Quit App" |
| 非表示 | `eye.slash` | トリガーをフェードアウトして非表示 | "Hide Trigger" |
| ログイン | `power` / `power.circle.fill` | ログインアイテムをトグル | "Enable/Disable Start at Login" |

**アクセシビリティ**: キーボードボタンは `accessibilityIdentifier("trigger-button")` を持つ。

#### アイコンホバーエフェクト
- スケール: 1.0 → 1.2 (ホバー時)
- グロー: 白色シャドウ (半径 8, 不透明度 0.3) (ホバー時)
- カラー: `#6B6B70` → `#FAFAFA` (ホバー時またはパネルアクティブ時)
...
ペンシルボタンをクリックすると、コンパクト行が編集カードに展開される。

| 要素 | サイズ | 説明 |
|------|------|------|
| アプリアイコン | 32×32 | 角丸 (9pt) |
| アプリ名 | — | 13pt、セミボールド |
| アプリディレクトリ | — | 10pt、`#4A4A50` |
| ショートカットバッジエリア | — | 3状態: 未設定、録画中、設定済み |
| キャンセルボタン (X) | 28×28 | 編集モードを終了 |
| デンジャートリガー (!) | 28×28 | アクションドロップダウンを開く |
...
| パネル | クラス | サイズ | 用途 |
|--------|--------|------|------|
| トリガー | `NSPanel` | 48×160 | 画面端ピルウィジェット |
| 拡張 | `KeyablePanel` | 300×360 | メイン操作パネル |
| ツールチップ | `NSPanel` | 動的 | ホバーツールチップ |

`KeyablePanel` は `NSPanel` サブクラスで、トグル可能な `canBecomeKey` によりショートカット録画時のキーボードフォーカスを有効化。
...
**動作**:
1. ビューがファーストレスポンダーになる → `KeyablePanel.allowBecomeKey = true`
2. ユーザーがモディファイア + キーを押す → `KeyboardShortcuts.setShortcut()` を呼び出し
3. Escape → 録画をキャンセル
4. ファーストレスポンダー解除 → 録画をキャンセル
5. キャプチャ/キャンセル後 → `KeyablePanel.allowBecomeKey = false`
